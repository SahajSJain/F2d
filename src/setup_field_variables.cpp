...
void setup_field_variables() //setup things like dx,dy,x,y,dx_inv... you get the idea
{
    ...
    // cout<<endl<<endl<<"dxinv is:"<<endl<<endl<<delx_inv;
    // cout<<endl<<endl<<"dx is:"<<endl<<endl<<delx;
    ...
    CFL = dt / del_min;
    outputout << "Smallest grid size is:" << del_min << endl;
    outputout << "CFL is:" << CFL << endl;
}
...
void setup_primitives()
{
    ...
    PresBC(New.p);
    VelBC(Old.u, Old.v);
    FaceFlux(Old.ci, Old.cj);
    PresBC(Old.p);
    VelBC(Older.u, Older.v);
    FaceFlux(Older.ci, Older.cj);
    PresBC(Older.p);
    // cout<<New.u_j;
