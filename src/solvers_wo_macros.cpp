...
#pragma omp parallel for private(j, i) shared(p, A, blank) collapse(2)
      for (j = j_s; j <= j_e; j++) {
        for (i = i_s; i <= i_e; i++) {

          p(i, j) = (1. - blank.blank(i, j)) *
                        ((1. - overrelax) * p(i, j) + overrelax * ptilde) +
                    blank.blank(i, j);
        }
      }
      PresBC(p);
...
#pragma omp parallel for private(j, i) collapse(2) shared(p, A, blank)         \
    reduction(+ : resp)
    for (j = j_s; j <= j_e; j++) {
      for (i = i_s; i <= i_e; i++) {
        resp =
